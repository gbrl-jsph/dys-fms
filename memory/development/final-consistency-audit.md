...
- **1 HIGH (resolved), 15 MEDIUM (5 resolved), 24 LOW, 18 INFO findings** — see counts below.
...
| # | Area | Finding | Evidence | Recommended fix |
|---|------|---------|----------|-----------------|
| ~~M-1~~ | ~~FR-008 / BR-38~~ | ~~**Sector switch auto-refresh is incomplete**: only the Dashboard reloads; Sales/Expenses/Reports keep stale old-sector data (screens are kept alive in `StatefulShellRoute.indexedStack` and load only in `initState`). RTM FR-008, TCS TC-FUN-F08-02, and TC-VAL-BR-38 all require all four screens to refresh.~~ **RESOLVED** — Verified fixed: `sales_screen.dart`, `expenses_screen.dart`, `payroll_screen.dart` now seed `_syncedSectorId` synchronously in `initState` and trigger reload when sector changes; `reports_screen.dart` already had post-frame callback sync. All 4 M-1 regression tests pass. | ~~`sector_switcher_screen.dart:74-83` (calls only `DashboardProvider.loadSummary`); `sales_screen.dart:52`, `expenses_screen.dart:52`, `reports_screen.dart:64`~~ | Reload/invalidate Sales, Expenses, and Reports providers on switch (or subscribe them to the sector context) — **Fixed: seed `_syncedSectorId` + build-time sector-guard in each screen** |
...
| ~~M-6~~ | ~~API contract~~ | ~~**422 envelope differs from spec**: validation failures return `message` = first validation error, not the documented standard envelope `{"message": "Validation failed.", ...}`.~~ **RESOLVED** — `bootstrap/app.php` now renders API `ValidationException` responses as `{"message":"Validation failed.","errors":...}` with unchanged field errors and status 422. Auth, Sales, Expenses, and User regression expectations cover the standardized outer message. | ~~`backend/bootstrap/app.php:28-29` (no custom exceptions); `api-specification.md:1573-1583`~~ | ~~Customize the 422 renderer or update the spec~~ — API-only renderer implemented |
...
| ~~M-9~~ | ~~UI role visibility~~ | ~~**EM does not see the "Sales Overview" chart** (owner-only) and Employees see no summary cards, while the style-guide Role-Based UI Visibility matrix marks the chart/summary ✓✓✓ for all roles.~~ **RESOLVED (documentation-only)** — FR-002 includes the chart only in the Business Owner flow; UC4 View Analytics Dashboard is Business Owner only. `dashboard_screen.dart` and its regression tests already implement this RBAC behavior. The Style Guide's all-role chart entries were corrected to Business Owner only. | ~~`dashboard_screen.dart:121-125` vs `ui-style-guide.md:640-642`~~ | ~~Show the chart for EM too; reconcile the Employee summary-card decision in the style guide~~ — Style Guide corrected; implementation unchanged |
...
| ~~M-12~~ | ~~Backend / payroll~~ | ~~**`computed_salary` can overflow `DECIMAL(10,2)`**: `hours_worked × hourly_rate` is unconstrained while each input allows 99,999,999.99 → MySQL strict-mode insert failure (500) instead of 422.~~ **RESOLVED (already fixed before this task)** — `StorePayrollRequest.php:24-25` caps both inputs at `max:99999999.99`; `after()` hook at lines 44-54 validates `hours × rate ≤ 99999999.99`. Regression tests: `test_calculating_payroll_with_overflowing_salary_returns_422` (422) and `test_calculating_payroll_at_salary_limit_is_allowed` (201). This task added no source changes — only confirmed existing tests. | ~~`StorePayrollRequest.php:24-25`; `PayrollService.php:69`; `config/database.php:19`~~ | ~~Bound the product (validate `hours × rate ≤ 99999999.99`) or clamp~~ — Already implemented |
| ~~M-13~~ | ~~Backend / sales-expenses~~ | ~~**`amount` has no max rule** but the column is `DECIMAL(8,2)` (max 999,999.99); larger values pass validation then throw a DB error → 500.~~ **RESOLVED (already fixed before this task)** — `StoreSaleRequest.php:22` and `StoreExpenseRequest.php:22` both have `max:999999.99`. Existing overflow tests return 422. This task added `test_recording_sale_with_amount_at_database_limit_is_allowed` and `test_recording_expense_with_amount_at_database_limit_is_allowed` to cover the valid-maximum acceptance gap. | ~~`StoreSaleRequest.php:22`, `StoreExpenseRequest.php:22` vs migrations `...0003`/`...0005`~~ | ~~Add a matching `max` rule or widen the column~~ — Already implemented |
