...
| Provider, GoRouter, Dio, Flutter Secure Storage, Laravel Sanctum | All are active dependencies and implementation components. Secure Storage applies to non-Web bearer tokens only. |
...
- Render runs the Dockerized Laravel API. The Aiven deployment requires CA-backed MySQL TLS certificate verification.
...
| Employee/Staff records expenses and views analytics | Employee/Staff may use authenticated profile/settings and sector-listing routes and view own payroll. Expense and report routes are denied. | Preserve current authorization. |
