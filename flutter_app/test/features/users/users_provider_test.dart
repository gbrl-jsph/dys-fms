...
void main() {
  ...
  DioException buildBadResponseException(int statusCode, Map<String, dynamic> data) {
  ...
  test('createUser surfaces the temporary password and refreshes the list',
      () async {
    ...
    expect(provider.state.error, isNull);
    expect(provider.state.successMessage, 'User account created successfully.');
  ...
  test('createUser sets an error message on failure', () async {
    fakeRepository.onCreateUser = (_) async {
      throw buildBadResponseException(
        422,
        {
  ...
  test('updateUserStatus sets a success message and refreshes the list',
      () async {
    ...
    expect(provider.state.successMessage, 'User status updated successfully.');
    expect(provider.state.error, isNull);
  });
  ...
  test('clearSuccess clears the success message and temporary password',
      () async {
    fakeRepository.onCreateUser = (request) async => UserAccount(
          id: 4,
          name: request.name,
