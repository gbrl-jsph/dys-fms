...
class _ExpensesScreenState extends State<ExpensesScreen> {
  ...
  void _submitSave(ExpensesProvider provider, {required bool isBusinessOwner}) {
    ...
    if (editingId != null) {
      ...
      provider.updateExpense(editingId, request, sectorId: sectorId).then((_) {
        if (!mounted) return;
  ...
  void _startEdit(ExpenseRecord record) {
    ...
    context.read<ExpensesProvider>().clearSuccess();
  }
        ...
        return AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                Text('ID: ${record.id}', style: textTheme.bodyMedium),
                ...
                if (record.updatedAt != null) ...[
                  ...
                  const SizedBox(height: 8),
                  Text(
                    'Updated: ${record.createdAt != null ? Formatters.formatDate(record.createdAt!) : Formatters.formatDate(record.recordedAt)}',
                    style: textTheme.bodyMedium,
                  ),
                ],
                if (record.payrollRecordId != null) ...[
  ...
  Widget build(BuildContext context) {
    ...
    final bool isEditing = _editingRecord != null;
        ...
        child: ListView(
          ...
          children: [
            ...
            _ExpensesList(
              ...
              onTap: (record) => _showExpenseDetails(record, isBusinessOwner),
            ),
            const SizedBox(height: AppSpacing.sp3),
            Text(
...
class _ExpensesList extends StatelessWidget {
  const _ExpensesList({required this.state, required this.onRetry, this.onTap});

  final ExpensesState state;
  final VoidCallback onRetry;
  final ValueChanged<ExpenseRecord>? onTap;
      ...
      child: Column(
        children: [
          for (int i = 0; i < state.expenses.length; i++) ...[
            ...
            _ExpenseRow(record: state.expenses[i], onTap: onTap),
          ],
        ],
      ),
    );
...
class _ExpenseRow extends StatelessWidget {
  const _ExpenseRow({required this.record, this.onTap});
